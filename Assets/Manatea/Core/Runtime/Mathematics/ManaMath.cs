...
namespace Manatea
{
	public static class ManaMath
	{
		...
		/// <summary>The inverse square root of two. One devided by the length of the vector (1,1)</summary>
		public const float SQRT2INV = 0.70710678119f;
		...
		/// <summary>Returns the angle of a vector. I don't recommend using this function, it's confusing~ Use ManaMath.DirToAng instead</summary>
		/// <param name="y">The y component of the vector. They're flipped yeah I know but this is how everyone implements it for some godforsaken reason</param>
		/// <param name="x">The x component of the vector. They're flipped yeah I know but this is how everyone implements it for some godforsaken reason</param>
		...
		/// <summary>Returns a vector where every dimension value is the smallest of the given values</summary>
		[MethodImpl(INLINE)]
		public static Vector2 Min(params Vector2[] v)
		{
			Vector2 output = v[0];
			for (int i = 1; i < v.Length; i++)
			{
				output.x = Min(output.x, v[i].x);
				output.y = Min(output.y, v[i].y);
			}
			return output;
		}
		...
		/// <inheritdoc cref="ManaMath.Min(Vector2[])"/>
		[MethodImpl(INLINE)]
		public static Vector3 Min(params Vector3[] v)
		{
			Vector3 output = v[0];
			for (int i = 1; i < v.Length; i++)
			{
		...
		[MethodImpl(INLINE)]
		public static Vector4 Min(params Vector4[] v)
		{
			Vector4 output = v[0];
			for (int i = 1; i < v.Length; i++)
			{
		...
		/// <summary>Returns a vector where every dimension value is the largest of the given values</summary>
		[MethodImpl(INLINE)]
		public static Vector2 Max(params Vector2[] v)
		{
			Vector2 output = v[0];
			for (int i = 1; i < v.Length; i++)
			{
				output.x = Max(output.x, v[i].x);
				output.y = Max(output.y, v[i].y);
			}
			return output;
		}
		...
		/// <inheritdoc cref="ManaMath.Max(Vector2[])"/>
		[MethodImpl(INLINE)]
		public static Vector3 Max(params Vector3[] v)
		{
			Vector3 output = v[0];
			for (int i = 1; i < v.Length; i++)
			{
		...
		[MethodImpl(INLINE)]
		public static Vector4 Max(params Vector4[] v)
		{
			Vector4 output = v[0];
			for (int i = 1; i < v.Length; i++)
			{
		...
		/// <summary>Applies a gamma curve or something idk I've never used this function before but it was part of Unity's original ManaMath.cs and it's undocumented</summary>
		...
		/// <inheritdoc cref="ManaMath.Lerp(Vector2,Vector2,Vector2)"/>
		...
		/// <inheritdoc cref="ManaMath.InverseLerp(Vector2,Vector2,Vector2)"/>
		...
		/// <seealso cref="ManaMathExtensions.Angle"/>
